...
type MatchDetailDTO = {
    ...
    conflictingHobbies: ConflictingHobby[],
...
export async function renderMatchComponent (partnerId? : string) {
    ...
    if(!userId) window.location.hash = "#error/UID-404";

    let partnerProfilesData : MatchDetailDTO[];
    let isPremium : boolean = false;

    if(partnerId && user.roles.some((role: string) => role === "Admin" || role === "Premium")){
        partnerProfilesData = await getMatchPremium(userId!, partnerId);
        isPremium = true;
    ...
    else if(!partnerId)
    {
        ...
        try {
            partnerProfilesData = await getGlobalMatch(userId!);
        } catch (error) {
    ...
    if(!partnerProfilesData) return;

    partnerProfilesData.forEach((item) => {
        if(item.finalScore === 0) {window.location.hash = "#error/chyba-dotazniku";}
    });
                                    ...
                                    <div class="row text-start gap-1 mx-2">
                                        ...
                                        ${partner.conflictingHobbies
                                                .filter(
                                                    (h, idx, arr) => 
                                                        arr.findIndex(
                                                            x => x.hobbyB.toLowerCase() === h.hobbyB.toLowerCase()
                                                        ) === idx
                                                ).map(
                                                (h: ConflictingHobby) => 
                                                    ` 
