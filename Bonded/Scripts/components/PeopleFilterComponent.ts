...
export async function renderPeopleFilterComponent() {
                    ...
                    <div class=" container filter-card mb-3 border-0 p-3 shadow-sm overflow-hidden">
                        ...
                        <div class="row mb-3 px-3">
                                    <div class="col-lg-6 col-md-6 my-2">
                                        <select class="form-select my_select my-color-primary" id="state" aria-label="select state" ${user.roles.some((role: string) => role === "Admin" || role === "Premium") ? "" : "disabled"}>
                                    ...
                                    <div class="col-lg-6 col-md-6 my-2">
                                        <select class="form-select my_select my-color-primary" id="city" aria-label="select city" ${user.roles.some((role: string) => role === "Admin" || role === "Premium") ? "" : "disabled"}>
  ...
  async function fetchFilteredUsers(
    ...
    function renderUserProfiles(users: any[]) {
                        ...
                        <div class="col mb-4">
                            <div class="card user-profile-card glassy text-center p-2 h-100 flex-column justify-content-between d-flex shadow-sm" data-chat-id="${user.id}">
                                <div>
                                    ...
                                    "https://placehold.co/60x60/87CEEB/ffffff?text=?"
                                    }" alt="" class="profile-avatar mb-2" style="width:80px; height:80px; border-radius:50%;">
                                    <div class="user-name-age fw-semibold">${
                                    user.firstName || ""
                                    } ${user.lastName || ""}</div>
                                    <div class="user-location text-muted mb-2"><i class="bi bi-geo-alt-fill me-1"></i>${
                                    user.city || ""
                                    }</div>
                                </div>
                                <div class="hobbies-list mt-2">
                                    ...
                                    user.hobbyNames && user.hobbyNames.length
                                        ? user.hobbyNames
                                            .map(
                                            (h: string) =>
                                                `<span class="badge rounded-pill text-bg-light mx-1 mb-1">${h}</span>`
                                            )
                                            .join("")
